...
SLIMER/GenQA 样本
  {
    ...
    "output": "[\"span_1\", \"span_2\"]"
        ...
        |      读取已生成的 PileNER train/dev JSONL，训练 Llama 3.1 LoRA，输出 trained_models/*
        |
...
- `requirements.txt`：运行依赖，包括 `torch`、`transformers`、`peft`、`vllm`、`huggingface-hub`、`datasets`、`numpy`、`openai`。LLaMA-Factory 是外部训练工具，按其官方文档在训练环境中单独安装。
- `.gitignore`：忽略本地环境文件、IDE 文件、生成的 `*.jsonl` 数据产物和本地 GPT guideline 生成产物。
- `LICENSE`：Apache 2.0。
...
该文件每行包含 `doc_tag_pairID`、`tagName`、`input`、`output`，其中 `output` 是 JSON list 字符串。脚本会跳过含非 ASCII 字符的样本，保证输出只含英文类字符。

`sample_pileNER_391_short_pos_full_neg_examples.py` 会从 `pileNER_391_all.jsonl` 中按实体类型随机抽取正例和原文负例，默认输出：

- `pileNER_391_3pos_2neg_examples.json`

该 JSON 按实体类型分组；正例优先从含 gold span 的文本中筛选短句，不足时回退到 `<200` 字符的含实体句，再不足时使用原始 `input` 全文；负例直接保留原始 `input` 全文。每条例句包含 `sentence`，实体 span 可用 `entities` 或旧字段 `target_words_in_it` 表示，prompt helper 会统一归一化。

`prompt_gpt_for_guidelines.ipynb` 会读取 `pileNER_391_3pos_2neg_examples.json` 并调用 OpenAI-compatible GPT API 生成层次化标注规范。notebook 从 `OPENAI_API_KEY` 读取密钥，默认 `BASE_URL` 为 `https://apix.ai-gaochao.cn/v1`，也可通过 `OPENAI_BASE_URL` 覆盖。默认输出：

- `pileNER_391_hierarchical_guidelines.json`

该输出按实体类型保存六字段 JSON：`entity_type`、`definition`、`include_rules`、`exclude_rules`、`boundary_rules`、`confusable_type_rules`。notebook 默认只处理 3 个实体类型作为 tiny-run，完整 391 类需要手动把 `MAX_ENTITY_TYPES` 改为 `None`。

`build_pileNER_391_5pos_5neg_with_guidelines.py` 会从 `pileNER_391_all.jsonl` 中按实体类型为 train/dev 各抽取最多 5 个正例和 5 个负例，并把 `pileNER_391_hierarchical_guidelines.json` 中对应的层次化标注规范写入完整 NER prompt 的 `instruction` 字段。默认输出：

- `pileNER_391_5pos_5neg_with_guidelines/train.jsonl`
- `pileNER_391_5pos_5neg_with_guidelines/dev.jsonl`

dev 抽样会跳过 exact `input` 已出现在 train 已选样本中的候选；数量不足时按实际可用数量写出。`instruction` 保留 `definition`、`include_rules`、`exclude_rules`、`boundary_rules`、`confusable_type_rules` 五个层次字段，并在前后加入单类型 NER 抽取和 JSON list 返回要求。

### `data/dataset_info.json`

LLaMA-Factory 的本地数据集注册文件。当前注册：

- `pilener_391_5pos_5neg_guidelines_train`：指向 `pileNER/pileNER_391_5pos_5neg_with_guidelines/train.jsonl`。
- `pilener_391_5pos_5neg_guidelines_dev`：指向 `pileNER/pileNER_391_5pos_5neg_with_guidelines/dev.jsonl`。

两个数据集都使用 Alpaca 格式列映射：`prompt -> instruction`、`query -> input`、`response -> output`。训练配置显式把 train 作为 `dataset`，把 dev 作为 `eval_dataset`；不从 train 自动切分 dev，也不读取任何 test 数据。
...
- `sample_pileNER_391_short_pos_full_neg_examples.py`：从 `data/pileNER/pileNER_391_all.jsonl` 中按实体类型抽取审查/构造 prompt 用示例；正例优先使用短句筛选逻辑并带回退策略，负例保留原始 `input` 全文。
- `build_pileNER_391_5pos_5neg_with_guidelines.py`：从无切分 391 类 JSONL 和层次化 guideline JSON 构造 train/dev 微调 JSONL；每类每 split 最多 5 正 5 负，dev 的 exact `input` 不与 train 已选样本重合，`instruction` 是带五个层次字段的完整抽取 prompt。
- `hierarchical_guideline_prompt.py`：构造层次化标注规范 prompt，输出 `entity_type`、`definition`、`include_rules`、`exclude_rules`、`boundary_rules`、`confusable_type_rules` 六字段 JSON，并提供回退到 SLIMER 平铺 `Definition` / `Guidelines` 的辅助函数。
- `prompt_gpt_for_guidelines.ipynb`：交互式调用 OpenAI-compatible GPT API 生成实体类型标注规范；直接复用 `hierarchical_guideline_prompt.py` 的 messages 构造函数，支持自定义 `OPENAI_BASE_URL`、单条测试、断点续跑和批量输出到 `data/pileNER/pileNER_391_hierarchical_guidelines.json`。
...
现有训练和评估转换逻辑仍消费平铺的 `Definition` / `Guidelines` 结构；层次化规范若要直接用于 SLIMER 指令，可先用 `flatten_guideline_for_slimer()` 将四类规则拼接回 `Guidelines`。
...
该目录包含历史 `transformers + peft` 训练配置，以及 LLaMA-Factory 原生训练/导出配置。

- `llama2_4_NER_XDef_NsamplesPerNE.yml`
- `llamafactory_lora_sft_pilener391_guidelines.yaml`
- `llamafactory_lora_sft_pilener391_guidelines_smoke.yaml`
- `llamafactory_export_pilener391_guidelines_lora.yaml`
...
LLaMA-Factory 配置使用本地 `../model/Llama-3.1-8B-Instruct`、`template: llama3`、fp16、LoRA rank 8、`q_proj,k_proj,v_proj`、`cutoff_len=2048`，默认面向单机两张 RTX 3090。完整训练输出到 `trained_models/Llama3_1_8B_pileNER_391_5pos_5neg_guidelines_lora`；smoke 配置只跑 64 条样本和 5 step，输出到同名 `_smoke` 目录；export 配置把 adapter 合并到 `merged_models/Llama3_1_8B_pileNER_391_5pos_5neg_guidelines_lora`。
...
LLaMA-Factory smoke test：

```bash
CUDA_VISIBLE_DEVICES=0,1 llamafactory-cli train src/SFT_finetuning/training_config/llamafactory_lora_sft_pilener391_guidelines_smoke.yaml
```

LLaMA-Factory 完整 LoRA 训练：

```bash
CUDA_VISIBLE_DEVICES=0,1 llamafactory-cli train src/SFT_finetuning/training_config/llamafactory_lora_sft_pilener391_guidelines.yaml
```

LLaMA-Factory LoRA 合并：

```bash
llamafactory-cli export src/SFT_finetuning/training_config/llamafactory_export_pilener391_guidelines_lora.yaml
```
...
该结构由 `src/data_handlers/hierarchical_guideline_prompt.py` 构造和校验。它用于生成或审查更细粒度的标注知识；如需接入现有 SLIMER 数据转换流程，应显式转换为平铺 D&G。
...
- `data/pileNER/pileNER_391_all.jsonl`：本地 PileNER conversation 文件转换出的无切分 391 类全集，不包含 `instruction` 字段，并过滤掉含非 ASCII 字符的样本。
- `data/pileNER/pileNER_391_3pos_2neg_examples.json`：按实体类型分组的示例 JSON；每类默认 3 个正例和最多 2 个原文负例，正例优先使用短句并在不足时回退；例句字段包含 `sentence`，实体 span 可用 `entities` 或旧字段 `target_words_in_it` 表示。
- `data/pileNER/pileNER_391_hierarchical_guidelines.json`：GPT API 生成的层次化 guideline JSON；notebook 增量写入并校验六字段 schema。
- `data/pileNER/pileNER_391_5pos_5neg_with_guidelines/`：`build_pileNER_391_5pos_5neg_with_guidelines.py` 生成的 train/dev JSONL；每行包含 `doc_tag_pairID`、`tagName`、`input`、`instruction`、`output`，其中 `instruction` 是带 `definition`、`include_rules`、`exclude_rules`、`boundary_rules`、`confusable_type_rules` 的完整抽取 prompt。
- `data/dataset_info.json`：LLaMA-Factory 本地数据集注册文件，指向 PileNER 391 类 5 正 5 负带 guideline 的 train/dev JSONL。
