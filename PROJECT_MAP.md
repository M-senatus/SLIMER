...
SLIMER/GenQA 样本
  {
    ...
    "output": "[\"span_1\", \"span_2\"]"
        ...
        |      训练 LoRA adapter，输出 trained_models/*
        |
        +--> src/SFT_finetuning/commons/merge_lora_weights.py
        |      合并 base model 和 LoRA，输出 merged_models/*
        |
...
6. 保存 adapter 到 `trained_models/LLaMA2_7B_*`，并复制训练配置。
...
该 base model 需要 Hugging Face 账号拥有 LLaMA 2 访问权限。
...
这些目录由脚本生成或期望存在，当前 `.gitignore` 未统一忽略它们：
...
如果要长期运行实验，建议明确这些目录是否应纳入版本管理或加入 `.gitignore`。
