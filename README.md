...
- `data/dataset_info.json`：LLaMA-Factory 本地数据集注册文件。
...
LLaMA-Factory 作为外部训练工具单独安装，不写入本仓库的 `requirements.txt`。建议在服务器训练环境中按 LLaMA-Factory 官方文档安装后确认命令可用：
...
使用 LLaMA-Factory 训练 Llama 3.1 8B Instruct LoRA。该入口读取已经生成的 `data/pileNER/pileNER_391_5pos_5neg_with_guidelines/train.jsonl` 和 `dev.jsonl`；`dev` 只作为 `eval_dataset`，不会从 train 自动切分，也不读取 test 标签。

先做 tiny smoke test，验证环境、数据注册和显存：
...
合并 LLaMA-Factory LoRA adapter：

```bash
llamafactory-cli export src/SFT_finetuning/training_config/llamafactory_export_pilener391_guidelines_lora.yaml
```

评估合并后的 LLaMA-Factory 模型：

```bash
PYTHONPATH=$(pwd) python src/SFT_finetuning/evaluating/evaluate_vLLM.py ./merged_models/Llama3_1_8B_pileNER_391_5pos_5neg_guidelines_lora LLaMA3-chat --with_guidelines
```
...
- `data/pileNER/`
- `data/dataset_info.json`：LLaMA-Factory 的本地 Alpaca 数据集注册文件，将 PileNER train/dev JSONL 映射到 `instruction`、`input`、`output` 三列。
...
- 当前原生 `transformers + peft` 训练配置和部分脚本名称仍保留历史 LLaMA 2 痕迹；运行该旧入口前，应先将对应配置或脚本中的 `base_model` 指向 `../model/Llama-3.1-8B-Instruct`，并将模板设为 `LLaMA3-chat`。新实验优先使用 `src/SFT_finetuning/training_config/llamafactory_lora_sft_pilener391_guidelines.yaml`。
- `llamafactory_lora_sft_pilener391_guidelines.yaml` 默认按单机两张 RTX 3090 配置，使用 fp16、LoRA rank 8、`cutoff_len=2048`。若 smoke test OOM，先把 smoke 和 full 配置中的 `cutoff_len` 降到 `1024` 后重试。
- LLaMA-Factory 训练前先运行 smoke test，不要直接启动完整训练。
